...
if ($post['mode'] == "build_code") {
	$dict_codes = $DB->selectCol('SELECT id AS ARRAY_KEY, code FROM ?_dict');
	...
	$code[] = (empty($post['schedule_id'])) ? "SSS": $dict_codes[$post['schedule_id']];
	$code[] = (empty($post['format_id'])) ? "FFF": $dict_codes[$post['format_id']];
	$code[] = (empty($post['age_id'])) ? "AAA": $dict_codes[$post['age_id']];
